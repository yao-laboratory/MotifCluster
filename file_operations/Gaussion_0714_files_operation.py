...
def write_score_result(res_path, data_axis, data_weight, final_data, p_score_final, data_count_sum, data_count_new):
    ...
    with open(res_path, 'w', newline='') as f:
        ...
        for i in range(len(p_score_final)):
            ...
            start_pos_head = center_pos_head - 8
            end_pos_head = center_pos_head + 8 + 1
            center_pos_tail = data_axis[id + cluster_size - 1]
            start_pos_tail = center_pos_tail - 8
            end_pos_tail = center_pos_tail + 8 + 1
            ...
            # cnt += 1
            csvwrite.writerow([i+1, center_pos_head, start_pos_head, center_pos_tail, end_pos_tail, cluster_size, class_id, max_weight, average_gap, score])
...
def write_weight_result(res_path2, data_weight_cluster, global_cluster_num):
    ...
    for i in range(global_cluster_num, 2 * global_cluster_num):
        mydict["cluster_length"+str(i-global_cluster_num)] = [len(data_weight_cluster[i-global_cluster_num])]
    # print(mydict)
...
#Gaussion_0711_cluster_merge.py           
def write_result(res_path, x1, y1, final_data):
