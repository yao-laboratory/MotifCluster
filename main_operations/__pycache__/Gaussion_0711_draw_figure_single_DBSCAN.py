...
def draw(input_file1, start_axis, end_axis):
    ...
    with open(final_filename) as f:
        ...
        for r in f_csv:
            ...
            data_weight.append(10)# 选择某一列加入到data数组中
            for cnt_temp2 in range(0, global_cluster_num):
                draw_input[cnt_temp2].append(row[cnt_temp2 + 2])
            arr_final.append(row[global_cluster_num + 2])
    ...
    value_total = []
    Fig,Axe=plt.subplots(global_cluster_num,1,sharex='col',sharey='row',figsize=(20,0.5))
    Axes = []
    Axes.append(Axe)
    ...
    #     draw_figure(draw_input_temp, 1, i, x1, y1, Axes)
    # print("second step:\n")
    ...
    print(arr_2)
    print(len(arr_2))
    draw_figure(arr_1,"final_2",0, x1, y1, Axes)

    # for i in range(global_cluster_num):
