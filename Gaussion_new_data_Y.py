...
def belong_which_cluster_better(cluster_0, cluster_1, id, distance, gm, f):
    temp_array = []
    for i in range(len(gm.means_)):
        # print(id,i)
    ...
    if len(temp_array) > 0:
        ...
        for i in range(len(temp_array) - 1):
            if abs(temp_array[i][2]) * 1.5 >= abs(temp_array[i+1][2]):
                if temp_array[i][1] == cluster_0 or temp_array[i][1] == cluster_1:
                    print("???????????????????????????")
                    print(temp_array[i][1])
...
# print(data_axis)
line_temp=[]
final_filename = "/home/eilene/Downloads/chrY.bed"
...
for i in range(0, len(gm.means_)):
    ...
    while(i0 < len(label_values)):
        if (data_axis[label_values[i0]] - 8) >= 125087000 and (data_axis[label_values[i0]] + 8 + 1) <= 125097000:
            label_space.append(i0)
            # print("************************")
            # print(i0)
        if (data_axis[label_values[i0]] + 8 + 1) > 125097000:
            break;
        i0 += 1
...
for i in range(len(data_axis)):
    if data_axis[i] >= 125087000 and data_axis[i] <= 125097000:
...
#     if cluster_belong[i-1] == cluster_belong[i] and i >= 1:
#     #not complete
#         num_place.append(i-1)
# intervals_cluster  = []      
# # for cnt in range(len(cluster_belong)):
# sum_count = 0
# for i in range(len(data_count)):
#     # if i > cluster_belong[cnt]:
#     #     break;
...
while i <= len(data_count) - 1:
    ...
    sum_count += data_count[i]
    distance = data_sum[sum_count] - data_sum[sum_count - 1]
    if i == 0 and cluster_belong[i] == -1:
        cluster_id = belong_which_cluster_better(cluster_belong[i], cluster_belong[i+1], i, distance, gm, f)
        if cluster_id == cluster_belong[i+1]:
            data_count_new.append(data_count[0] + data_count[1])   
            cluster_belong_new.append(cluster_belong[1])
            i += 1
            continue
    if i == len(data_count) - 1:
        break;
    if i + 1 == len(data_count) - 1:
        #not finished
        data_count_new.append(data_count[i])
        data_count_new.append(data_count[i+1])
        cluster_belong_new.append(cluster_belong[i])
        cluster_belong_new.append(cluster_belong[i+1])
        break;
    if i == len(data_count) - 1 and cluster_belong[i] == -1:
        cluster_id = belong_which_cluster_better(cluster_belong[i], cluster_belong[i+1], i, distance, gm, f)
        if cluster_id == cluster_belong[i+1]:
            data_count_new.append(data_count[i-1] + data_count[i])
            cluster_belong_new.append(cluster_belong[i-1])
    if cluster_belong[i] == cluster_belong[i+1] and cluster_belong[i] != -1:
        cluster_id = belong_which_cluster_better(cluster_belong[i], cluster_belong[i+1], i, distance, gm, f)
        if cluster_id == cluster_belong[i]:
            data_count_new.append(data_count[i] + data_count[i+1])
            cluster_belong_new.append(cluster_belong[i])
            set_value = True
    if cluster_belong[i+1] == -1 and cluster_belong[i] != -1 and cluster_belong[i+2] != -1: 
        if cluster_belong[i] == cluster_belong[i+2]:
            if belong_which_cluster_better(cluster_belong[i], cluster_belong[i+1], i, distance, gm, f) == belong_which_cluster_better(cluster_belong[i+1], cluster_belong[i+2], i, distance, gm, f):
                data_count_new.append(data_count[i] + data_count[i+1] + data_count[i+2])
                cluster_belong_new.append(cluster_belong[i])
            else:
                #???
                data_count_new.append(data_count[i] + data_count[i+1])
            ...
            #???
            cluster_id = belong_which_cluster_better(cluster_belong[i], cluster_belong[i+1], i, distance, gm, f)
            cluster_id_2 = belong_which_cluster_better(cluster_belong[i], cluster_belong[i+1], i, distance, gm, f)
            if cluster_id != -2 and cluster_id_2 != -2:
                data_count_new.append(data_count[i] + data_count[i+1])
                data_count_new.append(data_count[i+2])
                cluster_belong_new.append(cluster_belong[i])
                cluster_belong_new.append(cluster_belong[i+2])
                ...
                cluster_belong_new.append(cluster_belong[i+2])
        i += 2 
    elif cluster_belong[i+1] != -1:
          i += 1
    # if i + 2 == len(data_count) - 1:
    #     break;
print(data_count)
print(cluster_belong)
print("*******************")
...
#     for line in lines:
#         line = line.split()
#         if cluster_belong[line[0]] == line[1] or cluster_belong[line[0]+1] == line[1]:
