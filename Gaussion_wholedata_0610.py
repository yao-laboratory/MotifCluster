...
#draw the clusters using different cluster.bdg 
def draw_seperate_clusters(label_values, start_place, end_place, avg_show, cnt):
    # # del sample_weight
    # gc.collect
    label_space=[]
    label_drawing=[]
    ...
    while(i0 < len(label_values)):
        if (data_axis[label_values[i0]] - 8) >= start_place and (data_axis[label_values[i0]] + 8 + 1) <= end_place:
            label_space.append(i0)
            # #print("************************")
            # #print(i0)
        if (data_axis[label_values[i0]] + 8 + 1) > end_place:
            break;
        i0 += 1
    ...
    #print("----------------")
    x_draw = data_axis[label_values[label_space[0]]:label_values[label_space[-1]]+1]
    y_draw = data_weight[label_values[label_space[0]]:label_values[label_space[-1]]+1]
    print("===========================")
    print(label_values[label_space[0]])
    print(label_values[label_space[-1]]+1)
    #print(x_draw, y_draw)
    # #print(len(value_total))
    draw_figure(label_drawing, x_draw, y_draw, avg_show, cnt+1)    

#draw the clusters in using total.bdg
def draw_mergeclass_clusters(label_values, final_filename, start_place, end_place, avg_show, cnt):
    ...
    for i in range(len(data_axis)):
        if data_axis[i] >= start_place and data_axis[i] <= end_place:
    ...
    # #print(arr_temp)
    x_draw = data_axis[number_cnt[0]:number_cnt[-1]+1]
    y_draw = data_weight[number_cnt[0]:number_cnt[-1]+1]
    X_AXIS_DRAW.append(number_cnt[0])
    X_AXIS_DRAW.append(number_cnt[-1] + 1)
    print("^^^^^^^^^^^^^^^^^^^^^^^")
    print(number_cnt[0])
    print(number_cnt[-1]+1)
    print("^^^^^^^^^^^^^^^^^^^^^^^")
    #print(sequence)
    ...
    #print(x_draw)
    #print(len(x_draw))
    draw_figure(arr_final, x_draw, y_draw, "final",11)  
    # plt.tight_layout()    
    # plt.show()
...
def extend_xplot(x, y, x_maxsize, id):
    # #print("drawdrawdrawdrawdrawdraw")
    # #print(x)
    # #print(y)
    # #print("drawdrawdrawdrawdrawdraw")
    # plt.figure()
...
def draw_figure(data, x_draw, y_draw, id, num):
    ...
    for li in range(len(data)):
        # #print("&&&&&&&&&&&&&&&&&&&&")
        if data[li] == -1:
            # #print("x_draw outlier:")
            # #print(li,x_draw[li])
            x_outlier.append(x_draw[li])
            y_outlier.append(y_draw[li])
        else:
            # #print("nomal x_draw:")
            # #print(li,x_draw[li])
            x_normal.append(x_draw[li])
            y_normal.append(y_draw[li])
    # linestyle='--'
    # #print("outliers:")
    ...
    if num > 1:
        axn = plt.subplot(12,1,num,sharex=ax_draw, sharey=ax_draw)
    ...
    if num == 1:
        plt.setp(ax_draw.get_xticklabels(), fontsize=6)
    plt.subplots_adjust(wspace=0.5,hspace=0.5)
    # plt.tight_layout()
    # # plt.scatter(x_draw, y_draw, s=area, c=group_color, alpha=0.4, label='mean1')
    # # plt.legend()
...
for i in range(0, len(gm.means_)):
    ...
    draw_seperate_clusters(label_values, 6710000, 6724000, avg_show, cnt)
plt.tight_layout()   
# plt.show()
...
with open(final_filename,"r") as lines:
    ...
    for line in lines:
        ...
        for i in range(len(x_temp)):
            if int(x_temp[i]) > int(line[2]):
                break
            if int(x_temp[i]) >= int(line[1]) and int(x_temp[i]) <= int(line[2])-1:
                # if((len(arr_final) - 1) == i):
                #     continue
                arr_final.append(cluster_id)
                sequence.append(i)
...
# #print(len(x_draw))
# draw_figure(arr_final, x_draw, y_draw, "final",11)  
draw_mergeclass_clusters(label_values, final_filename, 6710000, 6724000, avg_show, cnt)
...
for i in range(len(data_count_new)):
    if data_count_new[i] == 1:
        arr_final_all.append(-1)
        continue
    cnt = data_count_new[i]
    for j in range(cnt):
        arr_final_all.append(count)
    count += 4
for cnt in range(len(arr_final_all)):
    if cnt >= X_AXIS_DRAW[0] and cnt < X_AXIS_DRAW[1]:
        arr_final_draw.append(arr_final_all[cnt])      
# print(arr_final_2)
# print(len(arr_f inal_2))
draw_figure(arr_final_draw,data_axis[X_AXIS_DRAW[0]:X_AXIS_DRAW[1]],data_weight[X_AXIS_DRAW[0]:X_AXIS_DRAW[1]],"final_2",12)  
plt.tight_layout()    
# plt.show()
...
for i in range(len(data_count_new)):
    if data_count_new[i] == 1:
        ...
        data_distance_temp_ = np.array([(int(data_axis[j+1]) - int(data_axis[j])) for j in range(total_start, total_start + data_count_new[i] - 1)])
...
for num in range(global_cluster_num):        
    ...
    print(len(data_weight_cluster[num]))
...
# # calculate the average distance of one cluster, then use this distance to calculate the score, is that right?
...
for i in range(len(p_score_final)):
    if i < 100:
        print(data_axis[data_count_sum[p_score_final[i][0]]])
