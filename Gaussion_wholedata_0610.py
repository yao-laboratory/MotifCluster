...
for i in range(len(data_count_new)):
    if data_count_new[i] == 1:
        arr_final_2.append(-1)
        continue
    cnt = data_count_new[i]
    for j in range(cnt):
        arr_final_2.append(count)
    count += 2
print(arr_final_2)
print(len(arr_final_2))
draw_figure(arr_final_2,"final_2",12)  
plt.tight_layout()    
plt.show()

print("start reading total file to place the oder:\n")
arr_final=[]
cluster_belong=[]
data_count=[]
data_sum=[]
...
for i in range(len(weight_sum)):
    count.append(0)
    for j in range(len(weight_sum)):
        if weight_sum[i] < weight_sum[j]:
            count[i] += 1
        if weight_sum[i] >= weight_sum[j]:
            probability_ig.append(count[i]/ len(weight_sum))
            continue
print(probability_ig)
p=[]


print(gm.predict_proba(X_DISTANCE))
for i in range(len(data_sum)):
    p_new  *=  gm.predict_proba(X_DISTANCE)[arr_final_2[i]] * probability_ig[i]
    if i == data_count_new[i]:
        p.append(p_new)
        p_new = 0
print(p)
