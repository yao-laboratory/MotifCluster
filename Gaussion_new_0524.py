...
for j in range(len(data_count_new)):
        ...
        print(j)
        print(cnt)
        data_count_sum.insert(cnt)

data_weight_cluster = [[] for i in range(global_cluster_num)]
for num in range(global_cluster_num):
    for id in range(len(cluster_belong_new)):
        if num == cluster_belong_new[id]:
            if id == 0:
                for i in range(len(data_count_sum[id])):
                    data_weight_cluster[num].append(data_weight[i])
            else:
                for i in range(len(data_count_sum[id]) - len(data_count_sum[id -1])):
                    data_weight_cluster[num].append(data_weight[len(data_count_sum[id -1]) + i])
...
probability_ig=[]
p_score=[]
p_g = -math.log(gm.score_samples(data.reshape(len(data), 1)))
for id in range(len(cluster_belong_new)):
    num_temp = cluster_belong_new[id]
    id_temp = id
    if (id_temp == 0):
        for i in range(len(data_count_sum[id])):
            count_1 = 0
            p_new = 0
            for id_cluster in range(len(data_weight_cluster[num_temp])):
                if data_weight[i] < data_weight_cluster[num_temp][id_cluster]:
                    count_1 += 1
                else:
                    probability_ig.append(-math.log(1 - count_1 / len(data_weight_cluster[num_temp])))
                    p_new  *=  p_g[i] * probability_ig[-1]
                    break
        ...
        for i in range(len(data_count_sum[id])):
            ...
            p_new = 0 
            for id_cluster in range(len(data_weight_cluster[num_temp])):
                if data_weight[len(data_count_sum[id -1]) + i] < data_weight_cluster[num_temp][id_cluster]:
                    count_2 += 1
                else:
                    probability_ig.append(-math.log(1 - count_2 / len(data_weight_cluster[num_temp])))
                    p_new  *=  p_g[len(data_count_sum[id -1]) + i] * probability_ig[-1]
                    break
            p_score.append([len(data_count_sum[id -1]) + i, p_new])

sorted(p_score, key=lambda s: s[1]) 
