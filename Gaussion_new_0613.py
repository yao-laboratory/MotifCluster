...
for id in range(len(cluster_belong_new)):
    num == cluster_belong_new[id]
    if num == -1:
        break
    if id == 0:
        for i in range(data_count_sum[id]):
            data_weight_cluster[num].append(float(data_weight[i]))
    else:
        for i in range(data_count_sum[id] - data_count_sum[id -1]):
            data_weight_cluster[num].append(data_weight[data_count_sum[id -1] + i])
